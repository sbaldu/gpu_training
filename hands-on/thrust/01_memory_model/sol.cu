...
int main() {
  ...
  thrust::sequence(thrust::host, h_buffer.begin(), h_buffer.end(), 0);
  ...
  thrust::copy(thrust::device, h_buffer.begin(), h_buffer.end(), d_buffer1.begin());

  // Part 4: copy the content of the first device buffer to the second device
  // buffer
  thrust::copy(thrust::device, d_buffer1.begin(), d_buffer1.end(), d_buffer2.begin());

  // Part 5: set all the values in the host buffer to zero
  thrust::fill(thrust::host, h_buffer.begin(), h_buffer.end(), 0);

  // Part 6: copy the contant back to the host buffer
  thrust::copy(thrust::device, d_buffer2.begin(), d_buffer2.end(), h_buffer.begin());
